...
async def add_album(response: Response, request: AlbumRq):
	...
	return AlbumResp(albumajdi = new_album_id, title = request.title, artistId = rq.artistId)
...
async def verify_album(album_id: int):
	...
	if len(data) == 0:
		raise HTTPException(status_code = 404, detail = {"error": "Item not found"})
	return AlbumResp(albumajdi = album_id, title = data[0]["title"], artistId = data[0]["artistId"])
